alter user root@localhost identified with  mysql_native_password by '1Q2W3E4R5T6Y';

DROP TRIGGER IF EXISTS `escuela`.`alumno_BEFORE_INSERT`;
use sakila;
...
##commit y rollback son para enviar o confirmar, devolver o desacer la accion en el start transaction
commit;
rollback;
select "after", payment_id, amount from payment where payment_id = 16;
select  "after", payment_id, amount from payment where payment_id = 10;
